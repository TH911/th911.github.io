...
如图，<span style="color:red">红色</span>部分冠以正号，<span style="color:blue">蓝色</span>部分被冠以负号：
...
记 $\tau(p_1,p_2,p_3)$ 为排列 $\langle p_1,p_2,p_3\rangle$ 的**逆序对**个数（有时也记作 $t(p_1p_2p_3)$），则 $3$ 阶行列式可写作：
...
<details class="note">
    <summary>答案解析</summary>
    <p>
        可以发现，要么可以出现 $x^3$ 项的排列只有两种：$\langle1,2,3,4\rangle,\langle1,2,4,3\rangle$。计算即可，系数为 $1-2=-1$。
    </p>
</details>
### 转置定理

记方阵 $A$ 的转置方阵为 $$A'$$（也记作 $A^T$）。实际上，**让 $A$ 关于其主对角线对称**即可得到 $A'$。
...
在定义式中，$p_i$ 对应的是**行标**，然而现在 $p_i$ 是列标。
...
$$
\sum_{\langle p_1,p_2,\cdots,p_n\rangle}(-1)^{\tau(p_1,p_2,\cdots,p_n)}a_{p_1,1}a_{p_2,2}a_{p_3,3}\cdots a_{p_n,n}=\sum_{\langle p_1,p_2,\cdots,p_n\rangle}(-1)^{\tau(p_1,p_2,\cdots,p_n)}a'_{1,p_1}a'_{2,p_2}a'_{3,p_3}\cdots a'_{n,p_n}
$$
...
在定义式中，即交换了排列中的 $p_1,p_2$，这会使 $\tau(p_1,p_2,p_3,\cdots,p_n)$ 发生 $1$ 的变化，则 $(p_1,p_2)$ 对行列式的值会乘上 $-1$。
...
其中，$D_j(1\leq j\leq n)$ 是把 $D_0$ 的第 $j$ 列替换为 $(b_1,b_2,\cdots,b_n)$ 得到的 $n$ 阶行列式，即：
...
* 当 $k=j$ 时，有：
  ...
  $$
  即将 $D_0$ 以第 $j$ 列展开，得到 $\sum\limits_{i=1}^na_{i,j}A_{i,j}$。

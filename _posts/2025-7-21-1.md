...
这样其实是避免了重复求解 $\textit{edge}_k$，而 $\textit{can}_{k,j}=[j\subseteq \textit{edge}_k]$。

得到 $\textit{can}_{k,j}$ 后，若 $\textit{can}_{k,j}=1$，就需要求解 $f_{k,j}$。
	...
	for(int i=1;i<=100;i++){
		...
		while(t>=eps){
			...
			do{
				...
				father[x]=y;
			}while(plCnt=0,count(root,plCnt),plCnt<n);
			int pl=calc(root);
			if(pl<ans){
				ans=pl;
				Min=min(Min,ans);

...
<details class="note">
    <summary>证明</summary>
    <details class="note">
        <summary>引理一：$(a+b)^n$ 的每一项的次数和均为 $n$</summary>
        <p>
            考虑数学归纳法。
        </p>
        <p>
    ...
    <p>
        想到得到 $a^ib^{n-i}$，即从 $n$ 个 $a+b$ 中选出 $i$ 个 $a$，即 $p_i=\dbinom{n}{i}$。
    </p>
    <p>
        故：
    </p>
...
\left\lfloor \frac np\right\rfloor\\
\left\lfloor \frac mp\right\rfloor
\end{pmatrix}$ 可以继续递归使用 Lucas 定理求解，而 $\dbinom{n\bmod p}{m\bmod p}$ 可以直接求解。
...
所以，也可以预处理模 $p$ 意义下的阶乘，然后使用乘法逆元 $\mathcal O(\log p)$ 计算 $\dbinom{n\bmod p}{m\bmod p}$。

这样的单次时间复杂度为：$\mathcal O(\log n\log p)$。

而预处理的时间复杂度为：$\mathcal O(p)$。

...
| 无 | 无 |能|$\left[x^n\right]G(x)=\frac{1}{(1-x)(1-x^2)\cdots(1-x^m)}$| 将 $n$ 个球分为 $1,2,3,\cdots,m$ 个非空部分，即分拆数。 |
| 无 | 无 |否|$\left[x^n\right]G(x)=\frac{x^m}{(1-x)(1-x^2)\cdots(1-x^m)}$| 将 $n$ 个球分为 $m$ 个非空部分，即分拆数。 |
...
将 $n$ 划分为 $k$ 个部分的拆分数，称为“$k$ 部分拆分数”，方案数记作 $p(n,k)$。
...
否则对于第二种情况，先给每一个部分都分配一个 $1$，然后将 $n-k$ 划分为 $k$ 个部分再加上即可。

...
> [参考资料](/file/2025/02/组合数学25年竞赛.zip) [参考资料](/file/2025/02/组合数学修订.zip) [参考资料](/file/2025/02/数学.pdf) [参考资料](/file/2025/02/母函数.pdf) [参考资料](/file/2025/02/组合数学0207.rar) [参考资料](/file/2025/02/组合数学0208.rar) [参考资料](/file/2025/02/组合数学0211.rar)
...
容斥原理**就是对于集合 $S_i$，明确元的属性 $P_i$ 从而确定 $S_i$**。

设总元素集合 $U$ 中存在 $n$ 种不同属性 $P_1,P_2,P_3,\cdots,P_n$，拥有属性 $P_i$ 的所有元素构成集合 $S_i$，则：
...
<details class="note">
    ...
    <p>
        $$
        \left\vert\bigcup_{i=1}^{m+1}S_i\right\vert=\left\vert\bigcup_{i=1}^mS_i\right\vert+\vert S_{m+1}\vert-\left\vert\left(\bigcup_{i=1}^mS_i\right)\bigcap S_{m+1}\right\vert
        $$
    </p>
    <p>
        展开交集 $\left\vert\left(\bigcup\limits_{i=1}^mS_i\right)\bigcap S_{m+1}\right\vert$，可得：
    </p>
    <p>
        $$
        \left\vert\left(\bigcup_{i=1}^mS_i\right)\bigcap S_{m+1}\right\vert=\sum_{1\leq i\leq k}\left\vert S_i\bigcap S_{m+1}\right\vert-\sum_{1\leq i<j\leq k}\left\vert S_i\bigcap S_j\bigcap S_{m+1}\right\vert+\cdots+(-1)^{m-1}\left\vert\bigcap_{i=1}^mS_i \bigcap S_{m+1}\right\vert
        $$
    </p>
    <p>
        回代，即可证明当 $n=m+1$ 时成立。    
...
$\varphi(n)$ 值的是 $\lbrace 1,2,3,\cdots,n\rbrace $ 中与 $n$ 互质的数的个数。
...
$$
\begin{aligned}
\varphi(n)&=\vert U\vert-\vert A_1\bigcup A_2\bigcup A_3\bigcup\cdots\bigcup A_n\vert\\
&=n\prod_{i=1}^k\frac{p_k-1}{p_k}
\end{aligned}
$$

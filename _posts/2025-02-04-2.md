...
* 划分至一个新的非空轮换，方案数 $$\begin{bmatrix}n-1\\k-1\end{bmatrix}$$。
* 划分至现有轮换，方案数 $$(n-1)\begin{bmatrix}n-1\\k\end{bmatrix}$$。
...
令 $f_i$ 表示前 $i$ 个元素的可能的出栈序列的数目

那么对于元素 $a_i$，其存在两种情况：

* 入栈后直接出栈，方案数为 $f_{i-1}$。
* 入栈后等待其后面入栈的元素入栈并出栈，其再次成为栈顶时出栈。

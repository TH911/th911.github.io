...
> [参考资料](/file/2025/02/组合数学25年竞赛.zip) [参考资料](/file/2025/02/组合数学修订.zip) [参考资料](/file/2025/02/数学.pdf) [参考资料](/file/2025/02/母函数.pdf) [参考资料](/file/2025/02/组合数学0207.rar) [参考资料](/file/2025/02/组合数学0208.rar)

# 待办事项

1. 扩展 Lucas 定理的代码
2. 排序生成算法：序数法、字典序法、换位法
3. 格路问题
4. Catalan 数
5. **Stirling 数**
6. 牛顿二项式定理
...
|     有     |     有     |     否     |             $m!\begin{Bmatrix}n\\m\end{Bmatrix}$             |                第二类 Stirling 数的一点改动。                |
|     有     |     无     |     能     | $\sum\limits_{i=1}^{\min(n,m)}\begin{Bmatrix}n\\i\end{Bmatrix}$ |          第二类 Stirling 数对所有可能非空盒数求和。          |
|     有     |     无     |     否     |              $\begin{Bmatrix}n\\m\end{Bmatrix}$              |                     第二类 Stirling 数。                     |
|     无     |     有     |     能     |                     $\dbinom{n+m-1}{n}$                      |          从 $m$ 个盒中选择 $n$ 个来放入球，可重复。          |
...
设满足条件的 $n$ 位数的个数未 $a_n$，则 $\langle a_n\rangle$ 的指数生成函数为 $G(x)$。
...
5. 第四条性质可逆，即 $\forall f_a\mid f_b,a|b$。
...
证明：参见[OI Wiki](https://oi-wiki.org/math/combinatorics/stirling/#%E9%80%9A%E9%A1%B9%E5%85%AC%E5%BC%8F)。

#### 同一行的第二类 Stirling 数计算

即 $n$ 确定，求 $\begin{Bmatrix}n\\0\end{Bmatrix},\begin{Bmatrix}n\\1\end{Bmatrix},\begin{Bmatrix}n\\2\end{Bmatrix},\cdots$。

根据通项公式**卷积**计算即可。

#### 同一列的第二类 Stirling 数计算

即 $k$ 确定，求 $\begin{Bmatrix}0\\k\end{Bmatrix},\begin{Bmatrix}1\\k\end{Bmatrix},\begin{Bmatrix}2\\k\end{Bmatrix},\cdots$。

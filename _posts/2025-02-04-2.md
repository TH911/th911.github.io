...
> [参考资料](/file/2025/02/组合数学25年竞赛.zip) [参考资料](/file/2025/02/组合数学修订.zip) [参考资料](/file/2025/02/数学.pdf) [参考资料](/file/2025/02/母函数.pdf) [参考资料](/file/2025/02/组合数学0207.rar)

# 待办事项

1. 排序生成算法：序数法、字典序法、换位法
2. 格路问题
3. 分拆数问题 Ferrers 图
4. 指数型母函数
5. Stirling 数
...
<div style="color:red;font-size:30px;">未完</div>
...
对于 $n$ 元集合 $S=\{a_1,a_2,a_3,\cdots,a_n\}$，若限定元素 $a_i$ 出现次数的集合为 $M_i$，则**该组合数序列的生成函数**为：
...
男同志只能选择偶数个，则能够选择的数目的集合为 $M_1=\{0,2,4,6,8\}$：

$$
\large G_1(x)=\binom80x^0+\binom82x^2+\binom84x^4+\binom86x^6+\binom88x^8
$$

女同志至少选择 $2$ 个，则能够选择的数目的集合为 $M_2=\{2,3,4,5\}$：
...
在这个例子中，就是一个二元集合 $S=\{\text{男同志},\text{女同志}\}$。

男同志能够出现的次数的集合 $M_1=\{0,2,4,6,8\}$，女同志能够出现的次数的集合 $M_2=\{2,3,4,5\}$。
...
<span style="color:red;font-size:30px">未完</span>
...
**类似于“$x$ 进制”，显然 $F(x)$ 乘上一个 $x$ 就会“右移一位”，空出最左边的 $x^0$ 的位置；但是对于无限序列，考虑到其长度为无限，因此当 $a_0,x>0$ 时 $x\cdot F(x)$ 反而可能小于 $F(x)$，因为少了  $x_0$ 的位置**。
...
<details class="note">
    <summary>关于“卷积后上升一位”</summary>
    <p>
        首先得了解“卷积”是什么。
    </p>
    <p>
        具体而言，就是对于两个序列 $a,b$，其<b>定义</b>卷积为：
    </p>
    <p>
        $$
    ...
    <p>
        而生成函数的卷积即多项式卷积。（生成函数的本质就是一个多项式）
    </p>
    <p>
        $\large \sum\limits_{i=0}^{n-1}f_if_{n-i}$ 可以看作 $f$ 与 $f$ 自己的卷积（自卷积）。
    </p>
    <p>
        然而，在上文的递推式中乘的不是 $f_{n-i}$，而是 $f_{n-i-1}$。
    </p>
    <p>
        而这完全可以看作是在生成函数中多乘了一个 $x$ 导致“右移”。
    </p>
    <blockquote>
        <b>类似于“$x$ 进制”，显然 $F(x)$ 乘上一个 $x$ 就会“右移一位”，空出最左边的 $x^0$ 的位置；但是对于无限序列，考虑到其长度为无限，因此当 $a_0,x>0$ 时 $x\cdot F(x)$ 反而可能小于 $F(x)$，因为少了  $x_0$ 的位置</b>。
    </blockquote>
</details>
...
$$
\begin{aligned}
F(x)&=\frac{(1\pm \sqrt{1-4x})(1\mp\sqrt{1-4x})}{2x(1\mp \sqrt{1-4x})}\\
&=\frac{1-(1-4x)}{2x(1\mp \sqrt{1-4x})}\\
&=\frac{4x}{2x(1\mp\sqrt{1-4x})}\\
&=\frac{2}{1\mp\sqrt{1-4x}}
\end{aligned}
$$

考虑到当 $x=0$ 时，$F(x)=G(x)=1$。

因此可列：

$$
\frac{2}{1\mp\sqrt{1}}=1
$$
...
所以这里的 $\large \mp$ 选择了 $\large +$，原来的 $\large \pm$ 应当选择 $\large -$。
...
其中，$A(x)$ 是序列 $a$ 的生成函数，$G(x)$ 是有限序列 $g$ 的生成函数，满足 $|g|\leq k$。

### 从生成函数的封闭形式求解通项公式

#### 通法

只讨论序列 $f$ 的生成函数为 $F(x)=\dfrac{\large1}{\large \sum\limits_{i=0}^ka_ix^i}$ 的形式。

不妨令 $\large \sum\limits_{i=0}^ka_ix^i=\prod\limits_{i=0}^t(1-p_ix)^{k_i}$，满足 $\large \sum\limits_{i=0}^tk_i=k$。

构造 $A_{i,j}$，则通过待定系数法可以解得：

$$
\dfrac{\large1}{\large \sum\limits_{i=0}^ka_ix^i}=\Large\sum_{i=0}^t\sum_{j=1}^{k_i}\large\frac{A_{i,j}}{(1-p_ix)^j}
$$
...
\begin{aligned}
F(x)&=\frac{x}{1-x-x^2}\\
&=\dfrac{\dfrac{1}{\sqrt5}}{1-\dfrac{1+\sqrt5}{2}\cdot x}\cdot\dfrac{-\dfrac{1}{\sqrt5}}{1-\dfrac{1-\sqrt5}{2}\cdot x}\\
&=
\end{aligned}
$$
